...
Panel {
  ...
  readonly property string pluginDir: "/home/sanjith/.config/omarchy/plugins/sanjith.power"
  ...
  Process {
    id: batteryProc
    command: [root.pluginDir + "/status.sh", "--shell"]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateKeyValue(text, "battery") }
  }

  Process {
    id: profilesProc
    command: [root.pluginDir + "/list.sh", "--active-state"]
          ...
          Row {
            ...
            Repeater {
              ...
              Button {
                ...
                text: modelData === "power-saver" ? "Eco" : (String(modelData).charAt(0).toUpperCase() + String(modelData).slice(1))
                tooltipText: modelData === "cool" ? "CPU Cool Mode (Eco CPU + Fan cooling)" : (modelData === "power-saver" ? "Power Saver (Eco mode)" : "")
